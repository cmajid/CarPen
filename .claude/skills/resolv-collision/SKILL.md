...
| Call | Use |
|---|---|
| `NewOBB(cx, cy, w, h, rotationDeg)` | Build a box **once** per entity. |
| `(*OBB).SetTransform(cx, cy, rotationDeg)` | Re-place it every tick — mutates in place, no allocation. |
| `Intersects(a, b *OBB) bool` | The SAT test. Strict: edge-touching boxes are not a hit. |
| `(*OBB).Corners() [4]Vector`, `Center()` | World-space geometry for the F3 debug overlay and tests. |
| `CollisionEvent{Obstruction}` | What detection raises; `ObstructionWall`, `ObstructionBush`, `ObstructionCar`. |

Entities own their boxes: `Car.OBB()` and `Bush.OBB()` derive centre/rotation from live state plus sprite size (kept as plain numbers so tests need no image) and return the same re-placed `*OBB` every call. Follow that pattern for anything new that collides.
...
- **F3** toggles the debug overlay in gameplay (hinted on the HUD): every box outlined, the active car's box **red for exactly the ticks it overlaps**, and a bottom strip with the live pivot/rotation/speed and `touching wall|bush|car` vs `clear` (`Gameplay.touching`). The crash notice under the HUD is different on purpose: it is the *first* crash remembered, the overlay strip is the truth of *now* — sprites have ≤6px transparent insets, so a "crashed while merely close" report is usually the 219px car's far corner clipping something, or the round bush's square box corners. Only the active car is checked against obstacles (parked things don't collide with each other).
